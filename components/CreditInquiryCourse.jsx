...
import {
  ArrowRight, BookOpen, Check, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, CircleDollarSign, CircleHelp,
...
export function CreditInquiryCourse({course}){
  ...
  return <div className="reader-shell inquiry-course"><div className="reader-progress"><span style={{width:`${scrollProgress}%`}}/></div><header className="reader-header"><button className="reader-menu-button" onClick={()=>setMenuOpen(true)} aria-label="Open course contents"><Menu/></button><Brand/><div className="reader-header-meta"><BookOpen size={17}/><span>Course 1.9</span><i/><span>{scrollProgress}% read</span></div><a className="reader-help" href="mailto:support@creditpulse.ca">Need help?</a></header><aside className={`reader-nav ${menuOpen?"open":""}`}><div className="reader-nav-head"><div><span>COURSE 1.9</span><strong>On this page</strong></div><button onClick={()=>setMenuOpen(false)}><X/></button></div><nav>{course19Sections.map((s,i)=><button key={s.id} className={activeSection===s.id?"active":""} onClick={()=>goToSection(s.id)}><span>{String(i+1).padStart(2,"0")}</span>{s.label}</button>)}</nav><div className="reader-nav-note"><LockKeyhole size={17}/><p><strong>Private course link</strong><span>Your progress stays on this device.</span></p></div></aside>{menuOpen&&<button className="reader-scrim" onClick={()=>setMenuOpen(false)}/>}<main className="reader-main">
    ...
    <section className="reader-section" id="action"><Heading number="06" title="Your Action (Fill Out Form): Application Decision Room"/><div className="checkin-intro"><div className="activity-reward-pill"><CircleDollarSign aria-hidden="true"/><span>Complete this activity and get <strong>$5</strong> plus <strong>20 Credit Pulse points</strong></span></div><p className="reader-lead">Scenario: Morgan plans to compare car loans, receives a prequalified credit-card email and is offered a store card at checkout.</p><p>Decide which door each situation belongs behind.</p></div>{complete?<div className="reader-completion"><span><Check/></span><p className="reader-kicker">COURSE 1.9 COMPLETE</p><h3>Nicely done{learner.name?`, ${learner.name.split(" ")[0]}`:""}.</h3><p>Your check-in is saved on this device.</p><button className="reader-primary" onClick={()=>window.print()}>Print course notes <FileCheck2/></button></div>:<div className="calculator-frame checkin-form-frame"><form className="checkin-form" onSubmit={submitActivity}><div className="checkin-identity"><label>Full name<input name="name" defaultValue={learner.name} required/></label><label>Email address<input name="email" type="email" defaultValue={learner.email} required/></label></div><label><span><b>11</b>OWN FILE — When Morgan checks her own report, is that hard or soft — and does it affect her score?</span><textarea name="ownFile" rows="3" required/></label><label><span><b>12</b>PREQUALIFICATION — What should Morgan ask before responding to the prequalified offer?</span><textarea name="prequalification" rows="3" required/></label><label><span><b>13</b>RATE SHOPPING — Why does comparing car-loan quotes within 12 days help Morgan's score?</span><textarea name="rateShopping" rows="3" required/></label><label><span><b>14</b>CHECKOUT OFFER — Before taking the $28 discount, what two things should Morgan check first?</span><textarea name="checkout" rows="3" required/></label><label><span><b>15</b>APPLICATION CLUSTER — Morgan already applied for two cards this month. Should she apply for a third?</span><textarea name="cluster" rows="3" required/></label><label><span><b>16</b>VAGUE SCREEN — The lender won't say what kind of check it's doing. Proceed, pause, or ask first?</span><textarea name="vague" rows="3" required/></label><label><span><b>17</b>UNFAMILIAR INQUIRY — What two things should Morgan do if she sees a hard inquiry she doesn't recognize?</span><textarea name="unfamiliar" rows="3" required/></label><label><span><b>18</b>APPLICATION FILTER — Finish the sentence: I'd accept a hard inquiry because this product ___, it costs ___, and I compared it to ___.</span><textarea name="filter" rows="3" required/></label><label className="checkin-consent"><input type="checkbox" required/><span>I reviewed my answers and understand that this is educational information, not financial advice.</span></label><button className="reader-primary">Complete Course 1.9 <ArrowRight/></button><small><ShieldCheck/>Do not include account numbers, passwords or sensitive identifiers.</small></form></div>}</section>
