...
import {
  ArrowRight, BellRing, BookOpen, Check, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight,
  CircleDollarSign, Clock3, CreditCard, FileCheck2, Landmark, Lightbulb,
  ListChecks, LockKeyhole, Menu, Plus, SearchCheck, ShieldCheck, WalletCards, X,
} from "lucide-react";
...
const filters = [
  ["Reporting", "Which Canadian credit bureau or bureaus receive account information?", SearchCheck],
  ["Total cost", "What are the annual, monthly, setup and other unavoidable fees? What interest rate applies if you carry a balance?", CircleDollarSign],
  ["Deposit", "Is one required? Where is it held, and how and when can it be returned?", Landmark],
  ["Payment", "Can you easily see the statement, due date, minimum payment and available payment methods?", CreditCard],
  ["Limit", "Is the limit small enough to manage comfortably but useful for your planned expense?", LockKeyhole],
  ["Exit or upgrade", "Can you close, change or graduate from the product without a nasty surprise?", ArrowRight],
  ["Purpose", "Does this solve a real need, or are you applying because the ad used words like “guaranteed” and a very confident font?", ShieldCheck],
];
...
const buildCarouselSlides = [
  { src: "/images/course-1-5/ca1.5.webp", alt: "Choosing one manageable starter product to build Canadian credit history." },
  { src: "/images/course-1-5/ca1.5.5.webp", alt: "Reviewing a credit report to confirm the new account is reporting correctly." },
];
...
function BuildImageCarousel() {
  ...
  useEffect(() => {
    if (paused) return;
    const interval = window.setInterval(() => setActiveSlide((current) => (current + 1) % buildCarouselSlides.length), 4000);
    return () => window.clearInterval(interval);
  }, [paused]);

  function showSlide(index) {
    setActiveSlide((index + buildCarouselSlides.length) % buildCarouselSlides.length);
  }

  return <section
    className="course-image-carousel build-image-carousel"
  ...
  >
    <div className="carousel-viewport">
      <div className="carousel-track" style={{ transform: `translateX(-${activeSlide * 100}%)` }}>
        {buildCarouselSlides.map((slide, index) => <div className="carousel-slide" role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${buildCarouselSlides.length}`} aria-hidden={activeSlide !== index} key={slide.src}>
          <Image
            src={slide.src}
            alt={slide.alt}
            fill
    ...
    <div className="carousel-dots" aria-label="Choose an image">{buildCarouselSlides.map((slide, index) => <button key={slide.src} type="button" className={activeSlide === index ? "active" : ""} onClick={() => showSlide(index)} aria-label={`Show image ${index + 1}`} aria-current={activeSlide === index ? "true" : undefined} />)}</div>
    <span className="carousel-status" aria-live="polite">Image {activeSlide + 1} of {buildCarouselSlides.length}</span>
  </section>;
}
...
export function BuildCreditCourse({ course }) {
  ...
  return <div className="reader-shell build-course">
    ...
    <main className="reader-main">
      <section className="reader-hero build-hero" id="start"><div className="reader-hero-media build-hero-media" aria-hidden="true"><Image src="/images/course-1-5/course1.5bg.webp" alt="" fill priority sizes="(max-width: 1050px) 100vw, calc(100vw - 272px)" /></div><div className="reader-hero-copy"><p className="reader-kicker">{course.eyebrow}</p><h1>{course.title}</h1><p className="reader-subtitle">{course.subtitle}</p><p className="reader-deck">The point of this lesson is to create useful credit history without turning your wallet into a science experiment.</p><div className="reader-meta"><span><Clock3 />{course.duration}</span><span><FileCheck2 />One course check-in</span></div><button className="reader-primary" onClick={() => goToSection("ladder")}>Build the first rung <ArrowRight /></button></div><div className="ladder-hero-visual" aria-hidden="true"><div className="credit-ladder">{[5,4,3,2,1].map((n) => <i key={n}><span>{n}</span></i>)}</div><div className="first-card"><span className="first-card-icon"><CreditCard /></span><span className="first-card-kicker">RUNG 01</span><strong>One manageable<br />account</strong></div></div></section>

      <div className="reader-body">
        <section className="reader-section reader-intro"><p className="reader-lead">Starting with little or no Canadian credit history can feel like being asked for job experience before anyone will give you the first job.</p><p>Mildly inconvenient? Yes. Permanent problem? No. The point of this lesson is to create useful credit history without turning your wallet into a science experiment.</p><p>Having limited Canadian credit history is not the same as having a bad history. It means lenders and credit bureaus have less Canadian credit activity to evaluate. FCAC notes that a poor or limited history can make borrowing harder, while your score changes over time as lenders update the information in your file.</p><div className="reader-links"><SourceLink href="https://www.canada.ca/en/financial-consumer-agency/services/credit-reports-score/credit-report-score-basics.html">FCAC: Credit report and score basics</SourceLink></div><div className="reader-context-media build-context-media"><div className="reader-context-copy"><Spotlight>No Canadian credit history is not the same thing as bad credit. You are not repairing a broken record; you are creating a useful record from the beginning.</Spotlight></div><BuildImageCarousel /></div></section>

        <section className="reader-section" id="ladder"><Heading number="01" title="The First Credit Ladder" /><p className="reader-lead">Your goal is not to race to the top. Build one rung that can safely hold your weight, then repeat the behaviour that makes the account useful.</p><ol className="ladder-steps">{ladder.map(([rung, title, copy], index) => <li key={rung}><span className="ladder-rung" aria-hidden="true"><span className="ladder-node">{index + 1}</span></span><div className="ladder-copy"><span className="ladder-kicker">{rung}</span><h3>{title} —</h3><p>{copy}</p></div></li>)}</ol><Spotlight>TransUnion’s guidance for building from scratch starts with “start slow”: one manageable card is more useful than several rushed applications that create new hard inquiries and new payment obligations.</Spotlight><div className="reader-links"><SourceLink href="https://www.transunion.ca/build-credit">TransUnion Canada: 5 Ways to Build Your Credit from Scratch</SourceLink></div></section>
        ...
        <section className="reader-section" id="payment"><Heading number="04" title="Partial Payment vs. Full Payment: What Changes?" /><p className="reader-lead">Paying at least the minimum by the due date meets the cycle’s minimum-payment requirement, but unpaid debt may generate interest. Paying the full statement balance generally avoids purchase interest when the card’s grace-period conditions apply.</p><p>For credit-building, the important lesson is not “full payment earns secret score points.” It is that on-time payment protects payment history, while a lower carried balance keeps debt and utilization easier to manage. The scoring formula is not a vending machine: inserting $23 of interest does not dispense 15 bonus points.</p><div className="reader-links"><SourceLink href="https://www.canada.ca/en/financial-consumer-agency/services/credit-cards/pay-off-credit-card.html">FCAC: Paying off your credit card</SourceLink><SourceLink href="https://www.canada.ca/en/financial-consumer-agency/services/credit-cards/credit-card-work.html">FCAC: How credit cards work — Interest-free grace periods</SourceLink></div><Spotlight>You do not need to carry a balance to prove you can use credit. Paying in full can avoid purchase interest; paying only the minimum can make repayment slower and more expensive.</Spotlight>
          <div className="story-card build-story"><div className="story-media" aria-hidden="true"><Image src="/images/course-1-5/story-img.webp" alt="" fill sizes="(max-width: 760px) 100vw, 860px" loading="eager" /></div><div className="story-content"><p className="reader-kicker">SEAN’S EXTREMELY UNEXCITING SUCCESS STORY</p><h3>Almost nothing dramatic happens.</h3><p>Sean opens his first Canadian credit card with a $1,000 limit. He is briefly tempted to celebrate by buying something worth $997, which would certainly make the card feel included. Instead, he gives it one job: his $55 monthly phone bill.</p><p>When the statement arrives, Sean checks the amount and due date, then pays the full $55 statement balance from money already set aside in his budget. He keeps the card active, makes payments reliably and checks the statement each month. Later, he reviews his credit report to confirm the account is appearing accurately.</p><p>What did Sean <em>not</em> do? He did not add shopping trips to “feed the score,” open three more cards because one felt lonely, or leave $11 unpaid just to donate interest to the cause. The story works precisely because almost nothing dramatic happens.</p></div></div><Spotlight>Credit-building is evidence over time. TransUnion says establishing a good credit history takes time; no legitimate provider can promise that one product will produce a particular score on a particular date.</Spotlight><div className="reader-links"><SourceLink href="https://www.transunion.ca/credit-score">TransUnion Canada: Credit score — Building a good credit history</SourceLink></div>
        </section>

        <section className="reader-section" id="quick-check"><Heading number="05" title="Quick Check: First-Rung Decisions" /><div className="quick-checks">{quickChecks.map(([prompt, answer], index) => <details key={prompt}><summary><span>{index + 6}</span><strong>{prompt}</strong><ChevronDown /></summary><div><b>{answer}</b></div></details>)}</div><h2 className="bureau-inline-heading">Your Action: Pick One First Rung</h2>
          <div className="sentence-builder">
            <div className="sentence-builder-card">
              <span className="sentence-builder-kicker">RESEARCH FIRST</span>
              <h3>Write your first-rung plan</h3>
              <p>Do not apply during this exercise. Choose the one route you would investigate and write down the three facts that would determine whether it deserves an application.</p>
            </div>
            <div className="sentence-builder-tiles">
              <article><span className="tile-plus"><Plus /></span><CreditCard /><b>Route to investigate</b></article>
              <article><span className="tile-plus"><Plus /></span><SearchCheck /><b>Fact to verify</b></article>
              <article><span className="tile-plus"><Plus /></span><CheckCircle2 /><b>Affordable expense</b></article>
            </div>
