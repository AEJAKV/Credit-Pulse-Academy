...
const publishedGoldRoutes = ["/course/gold/canada-child-benefit", "/course/gold/gst-hst-credit"];
...
function GoldCatalog() {
  return <div className="tier-content"><div className="tier-inheritance"><p className="tier-includes">Includes all Starter Courses</p><p className="tier-includes">Includes all Silver Courses</p></div>{goldCourses.map((course,courseIndex)=><details className="catalog-course" open={courseIndex===0} key={course.title}><summary><span>{`COURSE ${courseIndex+1}`}</span><div><strong>{course.title}</strong><small>{course.modules.length} modules · {courseIndex===0?"2 published":`${course.modules.length} planned`}</small></div><i aria-hidden="true">+</i></summary><ol className="module-list">{course.modules.map((title,moduleIndex)=>{const published=courseIndex===0&&moduleIndex<2;return <li className={published?"published":""} key={title}><span>{`${courseIndex+1}.${moduleIndex+1}`}</span><div><strong>{title}</strong><small>{published?"Published":"Planned"}</small></div>{published&&<a href={(courseIndex===0?publishedGoldRoutes:publishedGoldIncomeRoutes)[moduleIndex]} target="_blank" rel="noreferrer" aria-label={`Open ${title} learner view`}>Open ↗</a>}</li>})}</ol></details>)}</div>;
}
